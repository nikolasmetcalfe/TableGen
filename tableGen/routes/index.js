...
router.post('/login', function(req, res, next)
{
	...
	{
		if (!checkIfLetter(requestName[i]) && !checkIfNumber(requestName[i]))
		{
			res.redirect("/login");
			return;
		}
	}
	for (var i = 0; i < requestPass.length; i++)
	{
		if (!checkIfLetter(requestPass[i]) && !checkIfNumber(requestPass[i]))
		{
			res.redirect("/login");
			return;
		}
	...
	User.findOne({username: requestName}, function(err, result, count)
	{
		if (result === null)
		{
			req.session.username = requestName;
			name = requestName;
			new User
			({
		...
		{
			if (requestPass === result.password)
			{
				req.session.username = requestName;
				name = requestName;
				res.redirect('/generate');
...
router.get('/clearAll', function(req, res, next)
{
	...
	{		
		...
		{
			result.patches = [];
			result.markModified('patches');
			result.save(function(err, mod, count)
			{
				console.log("Deleted");
			});
		});
			
		Patch.remove({user: req.session.username}, function(err, result, count)
		{
...
{	
	...
	if (req.body.deleteBox === undefined)
	{
		res.redirect('patches');
	}
	if (req.body.deleteBox !== undefined)
	{
		if (!Array.isArray(req.body.deleteBox))
		{
		...
		{
			for (var i = 0; i < req.body.deleteBox.length; i++)
			{
...
{	
	...
	var randomizeAll = false;
	var userName = req.session.username;
	
	if (req.body.randomBox !== undefined)
		randomizeAll = true;
	
	if (randomizeAll)
	{
		patchObj = new Randomized(req.body.patchName + ""); //Original Constructor 1
		var nameCheck = patchObj.getName();
		if (nameCheck === undefined)
			patchObj.setName("");
		
		new Patch
		({
			volume: patchObj.volume,
			transpose : patchObj.transpose,
			commandOne : patchObj.command, 
			commandOneParams : patchObj.parameter, 
			commandTwo : patchObj.commandTwo, 
			commandTwoParams : patchObj.parameterTwo,
			name: patchObj.name,
			user: userName,
	...
	else
	{
		...
		cmd = cmd.map(mapCommands); // Higher Order Function 2
		cmdTwo = cmdTwo.map(mapCommands);
...
function Randomized(a)
{
	this.name = a;
	this.volume = newCol();
	this.transpose = newCol();
	this.command = newCom();
	this.parameter = newCol();
	this.commandTwo = newCom();
	this.parameterTwo = newCol();
}
...
function newCom()
{
	...
	for (var i = 0; i < 16; i++)
	{
		col[i] = validCmds[Math.floor(Math.random() * validCmds.length)];
		col[i] = col[i].toUpperCase();
	}
	return col;
}

Randomized.prototype.getName = function()
{
	return this.name;
}
Randomized.prototype.setName = function (a)
{
	this.name = a;
}
function mapCommands(a)
{
	if (a.length === 1)
	{
		if (!checkCommand(a))
		{
			a = validCmds[Math.floor(Math.random() * validCmds.length)];
		}
